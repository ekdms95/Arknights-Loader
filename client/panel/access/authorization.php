...
if (!isset($_SESSION["authorization_msg"])) 
{
    $_SESSION["authorization_msg"] = "";
}

if (isset($_POST["login"]) && isset($_POST["password"])) 
{
    if ($panel_access::authorization($_POST["login"], $_POST["password"]) != false)
    {
	    $_SESSION["auth"] = true;
	    $_SESSION["job"] = true;
    ...
    {
        $_SESSION["authorization_msg"] = "Incorrect login or password";
    }
}
?>
<!doctype html>
						...
						<div class="card">
							<div class="card-body p-6">						    
							    <form name="input" action="" method="post">
									<center><div class="card-title">Authorization</div></center>
									<div class="spacing"></div>
									<div class="form-group">
										<input type="text" name="login" id="login" class="form-control login" placeholder="Login">
									</div>
									<div class="form-group">
										<input type="password" name="password" id="password" class="form-control password" placeholder="Paswword">
									</div>
									<div class="spacing"></div>
									<div class="form-footer">
										<button type="submit" name="sub" class="btn btn-danger btn-block">Authorize</button>
									</div>
                                    <?php
                				    if ($_SESSION["authorization_msg"] != "") {
                				        
                				        echo  '<div class="spacing"></div> <center><h4 class="text-muted">' . $_SESSION["authorization_msg"] . '</h4></center>'; 
                				        $_SESSION["authorization_msg"] = "";
                				    }
                				    ?>	
						        </form>
						        <div class="spacing"></div>
						        <div class="text-center text-muted">Don't have a account? <a href="registration.php">Register</a></div>
                            </div>							
						</div>
					</div>
				</div>

...
<html lang="en" dir="ltr">
	<head>
		...
		<title>Manage global bans</title>
						...
						<div class="card">
							...
							<div class="card-body p-6">
							    <form name="input" action="" method="post">
							        <center><div class="card-title">Manage global bans</div></center>
									<div class="spacing"></div>
									<div class="form-group">
									    <center><div class='text-muted'> Hwid <br/></div></center>
                    					<select style="text-align: center; text-align-last: center;" name="hwid" class="form-control">
                        					<?php
                        					    $mysqli = mysqli_connect(HOST, USER, PASS, DB);
                        						$sql = "SELECT DISTINCT hwid FROM subscriptions";
                        						...
                        						if ($result->num_rows > 0)
                        						{
                        							while($row = $result->fetch_assoc()) 
                        							{
                        								...
                        								{
                        								    $hwid = $row["hwid"];
                                    						$sql2 = "SELECT * FROM global_bans WHERE hwid = '$hwid'";
                                    						$result2 = mysqli_query($mysqli, $sql2);                        								    
                                                            if ($result2->num_rows > 0) 
                            								{
                            								    $hwid_global_banned_format = " [global ban]";
                            									echo '<option style="text-align: left;" class="form-control">' . $row["hwid"] . $hwid_global_banned_format . "</option>";
                            									$_POST["hwid"] = str_replace("$hwid_global_banned_format", "" , $_POST["hwid"]);
                            								}
                        								    else 
                        								    {
                        								        echo '<option style="text-align: left;" class="btn btn-option btn-block">' . $row["hwid"] . "</option>";
                        								    }
                        								}
                        							}
                        						}
                        						...
                        						{
                        							echo '<option style="text-align: left;" class="form-control"> No hwid found. </option>';
                        						}
                        					?>
                    					</select>                    					
                    				</div>
    						        ...
    						        <?php 
                                        if (isset($_POST["hwid"]) && isset($_POST["action"]))
                                        {
                                            $panel_admin::manage_global_bans($_POST["hwid"], $_POST["action"]);

						...
						<div class="card">
							...
							<div class="card-body p-6">
							    <form name="input" action="" method="post">
							        <center><div class="card-title">Set secure cheat</div></center>
                    					...
                    					<select style="text-align: center; text-align-last: center;" name="cheat" class="form-control">
                        					<?php
                        						...
                        						if ($result->num_rows > 0)
                        						{
                        							...
                        							{
                        								if ($row["secure"] == "Undetected") 
                        								{
                        									echo '<option style="text-align: left; color: #5fb367;" class="form-control">'. $row["name"] . " - " . $row["secure"] . "</option>";
                        								}
                        								else if ($row["secure"] == "On update")
                        								{
                        								    echo '<option style="text-align: left; color: #3d85bf;" class="form-control">'. $row["name"] . " - " . $row["secure"] . "</option>"; 
                        								}
                        								else if ($row["secure"] == "Detected")
                        								{
                                                            echo '<option style="text-align: left;" class="form-control">'. $row["name"] . " - " . $row["secure"] . "</option>";          								    
                        								}
                    									$secure = $row['secure']; //pcode :(
                    									$_POST["cheat"] = str_replace(" - $secure", "" , $_POST["cheat"]);                        								
                        							}
                        						}
                        						else
                        						{
                					...
                					<div class="spacing"></div>
                					<center><div class='text-muted'> Secure <br/></div></center>
									<div class="form-group">
                    					<select style="text-align: center; text-align-last: center;" name="secure" class="form-control">
                    					    <option style="text-align: left; color: #5fb367;" class="form-control"> Undetected </option>
                    					    <option style="text-align: left; color: #3d85bf;" class="form-control"> On update </option>
                    					    <option style="text-align: left;" class="form-control"> Detected </option>
                    					</select>
                    				</div>
    									<div class="form-footer">
    										<button type="submit" name="sub" class="btn btn-danger btn-block">Set</button>
    									</div>                        				
									</div>
    						        <?php 
                                    if (isset($_POST["cheat"]) && isset($_POST["secure"]))
                                    {
                                        $panel_admin::set_secure_cheat($_POST["cheat"], $_POST["secure"]);

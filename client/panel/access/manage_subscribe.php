						...
						<div class="card">
							...
							<div class="card-body p-6">
							    <form name="input" action="" method="post">
							        <center><div class="card-title">Manage subscribe</div></center>
                        						...
                        						if ($result->num_rows > 0)
                        						{
                        							while($row = $result->fetch_assoc()) 
                        							{
                        								if ($row["license_key"] != null) 
                        								{
                                                            if ($row["banned"] == 1) 
                            								{
                            								    $key_banned_format_seller = " [banned] - " . $row["seller"];
                            									echo '<option style="text-align: left;" class="form-control">' . $row["license_key"] . $key_banned_format_seller . "</option>";
                            									$_POST["license_key"] = str_replace("$key_banned_format_seller", "" , $_POST["license_key"]);
                            								}
                        								    else 
                        								    {
                        								        $key_not_banned_format_seller = " - " . $row["seller"];
                        								        echo '<option style="text-align: left;" class="btn btn-option btn-block">' . $row["license_key"] . $key_not_banned_format_seller . "</option>";
                            									$_POST["license_key"] = str_replace("$key_not_banned_format_seller", "" , $_POST["license_key"]);
                        								    }
                        								}
                        							}
                        						}
									...
									<div class="form-group">
                    					<select style="text-align: center; text-align-last: center;" name="action" class="form-control">
                    					    ...
                    					    <option style="text-align: left;" class="form-control"> Reset hwid </option>
                    					</select>
                    				</div>
    						        ...
    						        <?php 
                                        if (isset($_POST["license_key"]) && isset($_POST["action"]))
                                        {
                                            $panel_admin::manage_subscribe($_POST["license_key"], $_POST["action"]);
